      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="220">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['project:prepare:detail']"
          >详情</el-button>
          <el-button
			v-if="scope.row.status==11 || scope.row.status==22 || scope.row.status==32 || scope.row.status==42"
            size="mini"
            type="text"
            icon="el-icon-edit"
            @click="handleUpdate(scope.row)"
            v-hasPermi="['project:prepare:edit']"
          >更新</el-button>
          <el-button
			v-if="scope.row.status==11 || scope.row.status==22 || scope.row.status==32 || scope.row.status==42"
            size="mini"
            ...
            v-hasPermi="['project:prepare:submit']"
          >提交</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { listPrepare, getBusProjectInfo, delBusProjectInfo, addBusProjectInfo, updateBusProjectInfo, exportBusProjectInfo,updatePorjectStatus } from "@/api/project/busProjectInfo";
